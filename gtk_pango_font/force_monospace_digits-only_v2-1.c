                        ...
                        const char align)
{
  ...
  if (max_width <= 0 || process_of_digit_needed == FALSE) return;
...
int main (int argc, char **argv) {
  ...
  PangoLayout *layout;
  gtk_init (&argc, &argv);
  const char* text = "_11:11:11_";//"Fr, Apr. 24\n14:35:14";//"_11:11:11_";//"_17:07:21_";;//"17:01:14";//"Fr Apr 24\n14:35:14";//01234iiw589\n115iiw12iwd12";
  ...
  */
  ...
  gboolean process_of_digit_needed = FALSE;
  guint32 glyphsNumIds[ TEST_STRING_LENGTH ];
  layout = gtk_label_get_layout (GTK_LABEL (labelRaw));
  font_check_digits_same_width (layout, &max_width, glyphsNumIds, &process_of_digit_needed);
  if (process_of_digit_needed == FALSE)
  {
    const char *textNotWork = "Warning!!! : This Demo does may NOT work your system.\nYou need to have install the font Cantarell. \nAlternative: set your application default font to a proportional font ((proportional for digits)).";
    g_print(textNotWork);
    gtk_label_set_text (GTK_LABEL(labelWelcomeHead),textNotWork);
  }


  if (process_of_digit_needed == TRUE)
  {
    convert_digits_to_same_width (label1, 'l');
    convert_digits_to_same_width (label2, 'c');
    convert_digits_to_same_width (label3, 'r');
  }
