/***
* Example / Demo:
* To get digits with the same width.
* e.g. convert proportional font to monospace font.
*
* Addational here: Two digits, which stand alone can bei align together (self-written).
*
...
// compile && run:
// gcc font_force_monospace_digits-only_align2together_v3-1.c `pkg-config --libs --cflags gtk+-3.0` && ./a.out
...
convert_digits_to_same_width (GtkWidget *label,
                        ...
                        int *max_width,
                        guint32 glyphsNumIds[TEST_STRING_LENGTH],
                        gboolean align_two_single_digits_nearby_together)
{
  ...
  //// this text-output here works with the text directly, and not with the glyphs: I think this this can potential be wrong   //ﬆ   https://en.wikipedia.org/wiki/Ligature_(writing) ????????????????
  gboolean isNumber [strlen (text)];
  int m=0;
  if (align_two_single_digits_nearby_together == TRUE)
  {
      ...
      for (r = line->runs; r; r = r->next)
      {
        ...
        for (i = 0; i < glyphs->num_glyphs; i++)
        {
          ...
          for (k = 0; k < TEST_STRING_LENGTH; k++)
            if ( glyphsNumIds[k] == glyphs->glyphs[i].glyph) isNumber[m] = TRUE;
          g_print("%i %c isDigit: ",m,text[m]); if(isNumber[m] == TRUE) g_print("Y\n"); else g_print("N\n");
          m++;
        }
      }
    }
        ...
        else
        {
          ...
          if (align_two_single_digits_nearby_together == TRUE)
          {
            ...
            if( isNumber[j] )
            {
              ...
              //first digit
              if (j<strlen(text)-1 && isNumber[j+1])
              { //next is number
                if( (j+2)>strlen(text) || !isNumber[j+2])
                { //next-next not exists || next-next is not number
                  if( (j-1)<0 || !isNumber[j-1])
                  { //prev not exists || prev is not number
                    g_print("found1\n");
                    align_cur='r';
                  }
                }
              ...
              if (j>0 )
              { //not first char
                if (j<strlen(text) && isNumber[j-1])
                { //prev is number
                  if( (j+1)>strlen(text) || !isNumber[j+1])
                  { //next not exists || next is not number
                    if( (j-2)<0 || !isNumber[j-2])
                    { //prev-prev not exists || prev-prev is not number
                      g_print("found2\n");
                      align_cur='l';
                    }
                  }
...
int main (int argc, char **argv) {
  ...
  GtkWidget *labelWelcomeHead, *labelHead, *labelRaw, *label0, *label1, *label2, *label3, *label4, *label_desc;
  PangoLayout *layout;
  gtk_init (&argc, &argv);
  const char* text = "_11:11:11_";//"abciww11s14w111d";"_17:07:21_";;//"17:01:14";//"Fr Apr 24\n14:35:14";//01234iiw589\n115iiw12iwd12";
  ...
  */
  ...
  gtk_container_add (GTK_CONTAINER(box), label4);
  ...
  */
  ...
  pango_context_set_font_description (context, desc);
  context = gtk_widget_get_pango_context (label4);
  pango_context_set_font_description (context, desc);
  ...
  convert_digits_to_same_width (label1, 'l', &max_width, glyphsNumIds, FALSE);
  convert_digits_to_same_width (label2, 'c', &max_width, glyphsNumIds, FALSE);
  convert_digits_to_same_width (label3, 'c', &max_width, glyphsNumIds, TRUE);
  convert_digits_to_same_width (label4, 'r', &max_width, glyphsNumIds, FALSE);
